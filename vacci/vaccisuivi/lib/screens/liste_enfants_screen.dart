...
class _ListeEnfantsScreenState extends State<ListeEnfantsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      setState(() {
        _enfants = _dataService.enfants;
        _villages = ['Tous', ...villages];
        _isLoading = false;
      });
  ...
  void _appliquerFiltres() async {
    List<Enfant> enfants = _enfants;

    // Filtre par village
    if (_filtreVillage != 'Tous') {
      try {
        enfants = await _dataService.getEnfantsByVillage(_filtreVillage);
      } catch (e) {
        enfants = _enfants.where((enfant) => enfant.village == _filtreVillage).toList();
      }
    }
    ...
    if (_filtreStatut != 'Tous') {
      try {
        enfants = await _dataService.getEnfantsByStatutVaccinal(_filtreStatut);
      } catch (e) {
